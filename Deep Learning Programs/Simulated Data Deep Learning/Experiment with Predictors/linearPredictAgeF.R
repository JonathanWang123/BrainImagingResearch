...
simu.f.age.train <- data.frame(simu.f.age.train)
model <- lm( age ~.,simu.f.age.train)
summary(model)

simu.f.age.test <- data.frame(simu.f.age.test)
pred <- predict(model,simu.f.age.test)

sum <- 0
for(x in 1:nrow(simu.f.age.test)){
  sum <- sum + abs(simu.f.age.test[x,221] - pred[x])
}
mae <- sum/nrow(simu.f.age.test)

paste0("Mean absolute error on test set with basic linear model: ", sprintf("%.2f", mae))