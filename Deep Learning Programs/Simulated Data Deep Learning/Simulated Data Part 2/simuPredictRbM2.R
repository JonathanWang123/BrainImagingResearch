...
simu.m.2.rb <- cbind(simu.m.2[,1:220],simu.m.2[,222])
colnames(simu.m.2.rb)[221] <- "rb"

#Split into train and test data
simu.m.2.rb.split <- crossValidate(simu.m.2.rb,seed = seed, trainPercentrb = 0.8)
names(simu.m.2.rb.split) <- c("train","test")

simu.m.2.rb.train <- simu.m.2.rb.split$train[,1:ncol(simu.m.2.rb.split$train)]
simu.m.2.rb.test <- simu.m.2.rb.split$test[,1:ncol(simu.m.2.rb.split$test)]

#Normalize Train Data
simu.m.2.rb.trainData <- as.data.frame(apply(simu.m.2.rb.train[,1:220],2,normalize))

#Normalize test data with respect to train data
simu.m.2.rb.testData <- c()
for (x in 1:220){
  simu.m.2.rb.testData <- cbind(simu.m.2.rb.testData,normalize(simu.m.2.rb.test[,x],y=simu.m.2.rb.train[,x]))
}
colnames(simu.m.2.rb.testData) <- colnames(simu.m.2.rb.trainData)
simu.m.2.rb.testData <- as.data.frame(simu.m.2.rb.testData)


#Format data to be used for training
simu.m.2.rb.trainTarget <- (simu.m.2.rb.train[,221])
simu.m.2.rb.testTarget <- (simu.m.2.rb.test[,221])

simu.m.2.rb.trainData <- data.matrix(simu.m.2.rb.trainData)
simu.m.2.rb.trainLabels <- data.matrix(simu.m.2.rb.trainTarget)
simu.m.2.rb.testData <- data.matrix(simu.m.2.rb.testData)
simu.m.2.rb.testLabels <- data.matrix(simu.m.2.rb.testTarget)

#Stop the training when val_loss doesn't change for 20 epochs
early_stop <- callback_early_stopping(monitor = "val_loss", patience = 20)

#Create Model
model.simu.m.2.rb<- keras_model_sequential()

model.simu.m.2.rb%>% #Architechture can be tweaked to change model
...
#Train Model
model.simu.m.2.rb%>% fit(
  simu.m.2.rb.trainData,
  simu.m.2.rb.trainLabels,
  epochs = 2000,
...
c(loss.m.2.rb, mae.m.2.rb) %<-% (model.simu.m.2.rb %>% evaluate(simu.m.2.rb.testData, simu.m.2.rb.testLabels, verbose = 0))

paste0("Mean absolute error on test set: ", sprintf("%.2f", mae.m.2.rb))

# Store predictions in a new dataset
predictions.simu.m.2.rb <- model.simu.m.2.rb%>% predict(simu.m.2.rb.testData, batch_size = 2374)
...
simu.m.2.rb.variableImportancePlot <- vip(
  object = model.simu.m.2.rb,          # fitted model
  method = "permute",                 # permutation-based VI scores,
  num_features = 10,       # top X features you want to keep track of (but ncol(simu.m.2.rb.trainData) for all variables)
  pred_wrapper = pred_wrapper,            # user-defined prediction function
  train = as.data.frame(simu.m.2.rb.trainData) ,    # training data
  target = simu.m.2.rb.trainLabels,                   # response values used for training
  metric = "rsquared",                # evaluation metric
  progress = "text"                 # request a text-based progress bar
)
print(simu.m.2.rb.variableImportancePlot)
print(simu.m.2.rb.variableImportancePlot[1])

#Get weights 
weights.m.2.rb <- get_weights(model.simu.m.2.rb)
