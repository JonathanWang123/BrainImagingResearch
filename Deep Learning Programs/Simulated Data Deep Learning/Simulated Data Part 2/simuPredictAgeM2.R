...
simu.m.2.age <- cbind(simu.m.2[,1:220],simu.m.2[,221])
colnames(simu.m.2.age)[221] <- "age"

#Split into train and test data
simu.m.2.age.split <- crossValidate(simu.m.2.age,seed = seed, trainPercentage = 0.8)
names(simu.m.2.age.split) <- c("train","test")

simu.m.2.age.train <- simu.m.2.age.split$train[,1:ncol(simu.m.2.age.split$train)]
simu.m.2.age.test <- simu.m.2.age.split$test[,1:ncol(simu.m.2.age.split$test)]

#Normalize Train Data
simu.m.2.age.trainData <- as.data.frame(apply(simu.m.2.age.train[,1:220],2,normalize))

#Normalize test data with respect to train data
simu.m.2.age.testData <- c()
for (x in 1:220){
  simu.m.2.age.testData <- cbind(simu.m.2.age.testData,normalize(simu.m.2.age.test[,x],y=simu.m.2.age.train[,x]))
}
colnames(simu.m.2.age.testData) <- colnames(simu.m.2.age.trainData)
simu.m.2.age.testData <- as.data.frame(simu.m.2.age.testData)


#Format data to be used for training
simu.m.2.age.trainTarget <- (simu.m.2.age.train[,221])
simu.m.2.age.testTarget <- (simu.m.2.age.test[,221])

simu.m.2.age.trainData <- data.matrix(simu.m.2.age.trainData)
simu.m.2.age.trainLabels <- data.matrix(simu.m.2.age.trainTarget)
simu.m.2.age.testData <- data.matrix(simu.m.2.age.testData)
simu.m.2.age.testLabels <- data.matrix(simu.m.2.age.testTarget)

#Stop the training when val_loss doesn't change for 20 epochs
early_stop <- callback_early_stopping(monitor = "val_loss", patience = 20)

#Create Model
model.simu.m.2.age<- keras_model_sequential()

model.simu.m.2.age%>% #Architechture can be tweaked to change model
...
#Train Model
model.simu.m.2.age%>% fit(
  simu.m.2.age.trainData,
  simu.m.2.age.trainLabels,
  epochs = 2000,
...
c(loss.m.2.age, mae.m.2.age) %<-% (model.simu.m.2.age %>% evaluate(simu.m.2.age.testData, simu.m.2.age.testLabels, verbose = 0))

paste0("Mean absolute error on test set: ", sprintf("%.2f", mae.m.2.age))

# Store predictions in a new dataset
predictions.simu.m.2.age <- model.simu.m.2.age%>% predict(simu.m.2.age.testData, batch_size = 2374)
...
simu.m.2.age.variableImportancePlot <- vip(
  object = model.simu.m.2.age,          # fitted model
  method = "permute",                 # permutation-based VI scores,
  num_features = 10,       # top X features you want to keep track of (but ncol(simu.m.2.age.trainData) for all variables)
  pred_wrapper = pred_wrapper,            # user-defined prediction function
  train = as.data.frame(simu.m.2.age.trainData) ,    # training data
  target = simu.m.2.age.trainLabels,                   # response values used for training
  metric = "rsquared",                # evaluation metric
  progress = "text"                 # request a text-based progress bar
)
print(simu.m.2.age.variableImportancePlot)
print(simu.m.2.age.variableImportancePlot[1])

#Get weights 
weights.m.2.age <- get_weights(model.simu.m.2.age)


weights.m.age.hiddenLayer2 <- weights.m.2.age[[3]]
