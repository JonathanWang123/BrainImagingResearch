...
simu.f.2.sensk <- cbind(simu.f.2[,1:220],simu.f.2[,224])
colnames(simu.f.2.sensk)[221] <- "sensk"

#Split into train and test data
simu.f.2.sensk.split <- crossValidate(simu.f.2.sensk,seed = seed, trainPercentsensk = 0.8)
names(simu.f.2.sensk.split) <- c("train","test")

simu.f.2.sensk.train <- simu.f.2.sensk.split$train[,1:ncol(simu.f.2.sensk.split$train)]
simu.f.2.sensk.test <- simu.f.2.sensk.split$test[,1:ncol(simu.f.2.sensk.split$test)]

#Normalize Train Data
simu.f.2.sensk.trainData <- as.data.frame(apply(simu.f.2.sensk.train[,1:220],2,normalize))

#Normalize test data with respect to train data
simu.f.2.sensk.testData <- c()
for (x in 1:220){
  simu.f.2.sensk.testData <- cbind(simu.f.2.sensk.testData,normalize(simu.f.2.sensk.test[,x],y=simu.f.2.sensk.train[,x]))
}
colnames(simu.f.2.sensk.testData) <- colnames(simu.f.2.sensk.trainData)
simu.f.2.sensk.testData <- as.data.frame(simu.f.2.sensk.testData)


#Format data to be used for training
simu.f.2.sensk.trainTarget <- (simu.f.2.sensk.train[,221])
simu.f.2.sensk.testTarget <- (simu.f.2.sensk.test[,221])

simu.f.2.sensk.trainData <- data.matrix(simu.f.2.sensk.trainData)
simu.f.2.sensk.trainLabels <- data.matrix(simu.f.2.sensk.trainTarget)
simu.f.2.sensk.testData <- data.matrix(simu.f.2.sensk.testData)
simu.f.2.sensk.testLabels <- data.matrix(simu.f.2.sensk.testTarget)

#Stop the training when val_loss doesn't change for 20 epochs
early_stop <- callback_early_stopping(monitor = "val_loss", patience = 20)

#Create Model
model.simu.f.2.sensk<- keras_model_sequential()

model.simu.f.2.sensk%>% #Architechture can be tweaked to change model
...
#Train Model
model.simu.f.2.sensk%>% fit(
  simu.f.2.sensk.trainData,
  simu.f.2.sensk.trainLabels,
  epochs = 2000,
...
c(loss.f.2.sensk, mae.f.2.sensk) %<-% (model.simu.f.2.sensk %>% evaluate(simu.f.2.sensk.testData, simu.f.2.sensk.testLabels, verbose = 0))

paste0("Mean absolute error on test set: ", sprintf("%.2f", mae.f.2.sensk))

# Store predictions in a new dataset
predictions.simu.f.2.sensk <- model.simu.f.2.sensk%>% predict(simu.f.2.sensk.testData, batch_size = 2374)
...
simu.f.2.sensk.variableImportancePlot <- vip(
  object = model.simu.f.2.sensk,          # fitted model
  method = "permute",                 # permutation-based VI scores,
  num_features = 10,       # top X features you want to keep track of (but ncol(simu.f.2.sensk.trainData) for all variables)
  pred_wrapper = pred_wrapper,            # user-defined prediction function
  train = as.data.frame(simu.f.2.sensk.trainData) ,    # training data
  target = simu.f.2.sensk.trainLabels,                   # response values used for training
  metric = "rsquared",                # evaluation metric
  progress = "text"                 # request a text-based progress bar
)
print(simu.f.2.sensk.variableImportancePlot)
print(simu.f.2.sensk.variableImportancePlot[1])

#Get weights 
weights.f.2.sensk <- get_weights(model.simu.f.2.sensk)
