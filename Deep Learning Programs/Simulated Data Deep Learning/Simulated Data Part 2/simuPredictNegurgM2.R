...
simu.m.2.negurg <- cbind(simu.m.2[,1:220],simu.m.2[,223])
colnames(simu.m.2.negurg)[221] <- "negurg"

#Split into train and test data
simu.m.2.negurg.split <- crossValidate(simu.m.2.negurg,seed = seed, trainPercentnegurg = 0.8)
names(simu.m.2.negurg.split) <- c("train","test")

simu.m.2.negurg.train <- simu.m.2.negurg.split$train[,1:ncol(simu.m.2.negurg.split$train)]
simu.m.2.negurg.test <- simu.m.2.negurg.split$test[,1:ncol(simu.m.2.negurg.split$test)]

#Normalize Train Data
simu.m.2.negurg.trainData <- as.data.frame(apply(simu.m.2.negurg.train[,1:220],2,normalize))

#Normalize test data with respect to train data
simu.m.2.negurg.testData <- c()
for (x in 1:220){
  simu.m.2.negurg.testData <- cbind(simu.m.2.negurg.testData,normalize(simu.m.2.negurg.test[,x],y=simu.m.2.negurg.train[,x]))
}
colnames(simu.m.2.negurg.testData) <- colnames(simu.m.2.negurg.trainData)
simu.m.2.negurg.testData <- as.data.frame(simu.m.2.negurg.testData)


#Format data to be used for training
simu.m.2.negurg.trainTarget <- (simu.m.2.negurg.train[,221])
simu.m.2.negurg.testTarget <- (simu.m.2.negurg.test[,221])

simu.m.2.negurg.trainData <- data.matrix(simu.m.2.negurg.trainData)
simu.m.2.negurg.trainLabels <- data.matrix(simu.m.2.negurg.trainTarget)
simu.m.2.negurg.testData <- data.matrix(simu.m.2.negurg.testData)
simu.m.2.negurg.testLabels <- data.matrix(simu.m.2.negurg.testTarget)

#Stop the training when val_loss doesn't change for 20 epochs
early_stop <- callback_early_stopping(monitor = "val_loss", patience = 20)

#Create Model
model.simu.m.2.negurg<- keras_model_sequential()

model.simu.m.2.negurg%>% #Architechture can be tweaked to change model
...
#Train Model
model.simu.m.2.negurg%>% fit(
  simu.m.2.negurg.trainData,
  simu.m.2.negurg.trainLabels,
  epochs = 2000,
...
c(loss.m.2.negurg, mae.m.2.negurg) %<-% (model.simu.m.2.negurg %>% evaluate(simu.m.2.negurg.testData, simu.m.2.negurg.testLabels, verbose = 0))

paste0("Mean absolute error on test set: ", sprintf("%.2f", mae.m.2.negurg))

# Store predictions in a new dataset
predictions.simu.m.2.negurg <- model.simu.m.2.negurg%>% predict(simu.m.2.negurg.testData, batch_size = 2374)
...
simu.m.2.negurg.variableImportancePlot <- vip(
  object = model.simu.m.2.negurg,          # fitted model
  method = "permute",                 # permutation-based VI scores,
  num_features = 10,       # top X features you want to keep track of (but ncol(simu.m.2.negurg.trainData) for all variables)
  pred_wrapper = pred_wrapper,            # user-defined prediction function
  train = as.data.frame(simu.m.2.negurg.trainData) ,    # training data
  target = simu.m.2.negurg.trainLabels,                   # response values used for training
  metric = "rsquared",                # evaluation metric
  progress = "text"                 # request a text-based progress bar
)
print(simu.m.2.negurg.variableImportancePlot)
print(simu.m.2.negurg.variableImportancePlot[1])

#Get weights 
weights.m.2.negurg <- get_weights(model.simu.m.2.negurg)
