...
dt1su18 <- cbind(dt1[,1],dt1[,6])
colnames(dt1su18) <- c("Subject","su.18")
dt2su18 <- cbind(dt2[,1],dt2[,6])
colnames(dt2su18) <- c("Subject","su.18")
dt4su18 <- cbind(dt4[,1],dt4[,4])
colnames(dt4su18) <- c("Subject","su.18")
trimmedFreesurfer <- trimByCol(freesurfer,trimVars)
trimmedFreesurfer <- merge(dt1su18,trimmedFreesurfer, by.x = "Subject",by.y = "Subject")
trimmedFreesurfer <- na.omit(trimmedFreesurfer)

hcp.su18.dt1.split <- crossValidate(trimmedFreesurfer,seed = seed, trainPercentage = 1)
names(hcp.su18.dt1.split) <- c("train","test")

hcp.su18.dt1.train <- hcp.su18.dt1.split$train[,2:ncol(hcp.su18.dt1.split$train)]
hcp.su18.dt1.test <- hcp.su18.dt1.split$test[,2:ncol(hcp.su18.dt1.split$test)]

#Normalize Train Data
hcp.su18.dt1.trainData <- as.data.frame(apply(hcp.su18.dt1.train[,2:224],2,normalize))

#Normalize test data with respect to train data
hcp.su18.dt1.testData <- c()
for (x in 2:224){
  hcp.su18.dt1.testData <- cbind(hcp.su18.dt1.testData,normalize(hcp.su18.dt1.test[,x],y=hcp.su18.dt1.train[,x]))
}
colnames(hcp.su18.dt1.testData) <- colnames(hcp.su18.dt1.trainData)
hcp.su18.dt1.testData <- as.data.frame(hcp.su18.dt1.testData)

#Format data to be used for training
hcp.su18.dt1.trainTarget <- to_categorical(hcp.su18.dt1.train[,1])
hcp.su18.dt1.testTarget <- to_categorical(hcp.su18.dt1.test[,1])

hcp.su18.dt1.trainData <- data.matrix(hcp.su18.dt1.trainData)
hcp.su18.dt1.trainLabels <- data.matrix(hcp.su18.dt1.trainTarget)
hcp.su18.dt1.testData <- data.matrix(hcp.su18.dt1.testData)
hcp.su18.dt1.testLabels <- data.matrix(hcp.su18.dt1.testTarget)
...
#Create Model
model.hcp.su18.dt1<- keras_model_sequential()

model.hcp.su18.dt1%>% #Architechture can be tweaked to change model
  layer_dense(units = 223, activation = 'relu', kernel_initializer='he_normal',bias_initializer ='he_normal', input_shape = c(223)) %>% 
  layer_dense(units = 2, activation = 'sigmoid')

model.hcp.su18.dt1%>% compile(
...
#Train Model
model.hcp.su18.dt1%>% fit(
  hcp.su18.dt1.trainData,
  hcp.su18.dt1.trainLabels,
  epochs = 250,
...
hcp.su18.dt1.predictions <- model.hcp.su18.dt1%>% predict_classes(hcp.su18.dt1.trainData, batch_size = 48)
# Confusion matrix
hcp.su18.dt1.confusion <- table(hcp.su18.dt1.train[,1], hcp.su18.dt1.predictions)

hcp.su18.dt1.correct <- 0
hcp.su18.dt1.incorrect <- 0
for(row in 1:nrow(hcp.su18.dt1.confusion)) {
  for(col in 1:ncol(hcp.su18.dt1.confusion)) {
    if(rownames(hcp.su18.dt1.confusion)[row] == colnames(hcp.su18.dt1.confusion)[col]){
      hcp.su18.dt1.correct <- sum(hcp.su18.dt1.correct,hcp.su18.dt1.confusion[row,col])
    } else {
      hcp.su18.dt1.incorrect <- sum(hcp.su18.dt1.incorrect,hcp.su18.dt1.confusion[row,col])
    }
  }
}
cat("The accuracy of the multilayer perceptron is", 100*(hcp.su18.dt1.correct/(hcp.su18.dt1.correct+hcp.su18.dt1.incorrect)),"%")
...
train_data_frame <- as.data.frame(hcp.su18.dt1.trainData)
test_data_frame <- as.data.frame(hcp.su18.dt1.testData)
predict_model (x       = model.hcp.su18.dt1, 
...
explainer <- lime::lime (
  x              = train_data_frame, 
  model          = model.hcp.su18.dt1, 
  bin_continuous = FALSE)

hcp.su18.dt1.explanation <- lime::explain (
  train_data_frame[1:10, ], 
  explainer    = explainer, 
  n_labels     = 2, 
  n_features   = 223, 
  kernel_width = 0.5)
plot_features(hcp.su18.dt1.explanation[1:50,]) 
plot_explanations (hcp.su18.dt1.explanation[1:50,]) 

#Get Weights
weights.hcp.su18.dt1 <- get_weights(model.hcp.su18.dt1)
